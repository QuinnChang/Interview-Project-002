...
memberManager.func.createTabs = function () {
  let target = $("#tab");
  let butttonList = toolBox.func.addButton(target, {
    id: "buttonList",
    class: "tablinks",
  });
...
memberManager.func.defultOption = function () {
  ...
  $("#submit").on("click", function (e) {
    ...
    if (valueDateMon == "") return alert("請輸入日期");
    if (valueType == "請選擇部位") return alert("請輸入部位");
    if (valueWay == "請選擇方式") return alert("請輸入方式");
    if (valueWeight == "") return alert("請輸入重量");
    if (valueReps == "") return alert("請輸入次數");
    if (valueSets == "") return alert("請輸入組數");
    if (valueRest == "") return alert("請輸入休息時間");
...
memberManager.func.drawLession = function () {
  ...
  for (const key of dataBaseMap.keys()) {
    let listFrame = toolBox.func.addDiv($("#list"), {
      id: key,
      class: "listFrame",
    });
    ...
    for (let i = 0; i < value.length; i++) {
      let itemID = key + "-" + i;
      let arrIndex = i;
      let listItem = toolBox.func.addDiv($(listFrame), {
        id: itemID,
        class: "item",
      });
      let listTypeWayDiv = toolBox.func.addDiv($(listItem), {
        class: "listTypeWayDiv",
      });
      ...
      let listNumDiv = toolBox.func.addDiv($(listItem), {
        class: "listNumDiv",
      });
      ...
      let listBtnEdit = toolBox.func.addButton($(listNumDiv), {
        class: "edit",
      });
      $(listBtnEdit).text("編輯");
      ...
      let listBtnDel = toolBox.func.addButton($(listNumDiv), {
        class: "delet",
      });
      $(listBtnDel).text("刪除");
...
memberManager.func.drawEditLessionFrame = function (key, arrIndex) {
  ...
  let editFrameTitle = toolBox.func.addDiv($(editFrame), {
    id: "editFrameTitle",
  });
  $(editFrameTitle).text("會員課表更新");

  let editTypeDiv = toolBox.func.addDiv($(editFrame), {
    id: "editTypeDiv",
    class: "div",
  });
  let editFrameTypeLable = toolBox.func.addDiv($(editTypeDiv), {
    class: "lable",
  });
  ...
  let typeListIndex = memberManager.constants.typeList.indexOf(
    dataBaseMap.get(key)[arrIndex].type
  );
  $("#editFrameType option")[typeListIndex].selected = true;

  let editWayDiv = toolBox.func.addDiv($(editFrame), {
    id: "editWayDiv",
    class: "div",
  });
  let editFrameWayLable = toolBox.func.addDiv($(editWayDiv), {
    class: "lable",
  });
  ...
  let wayListIndex = memberManager.constants.waysList[typeListIndex].indexOf(
    dataBaseMap.get(key)[arrIndex].way
  );
  for (
    let j = 0;
    j < memberManager.constants.waysList[typeListIndex].length;
    j++
  ) {
    $("#editFrameWays").append(
      new Option(
  ...
  let editWeightDiv = toolBox.func.addDiv($(editFrame), {
    id: "editWeightDiv",
    class: "div",
  });
  ...
  let editRepsDiv = toolBox.func.addDiv($(editFrame), {
    id: "editRepsDiv",
    class: "div",
  });
  ...
  let editSetsDiv = toolBox.func.addDiv($(editFrame), {
    id: "editSetsDiv",
    class: "div",
  });
  ...
  let editRestDiv = toolBox.func.addDiv($(editFrame), {
    id: "editRestDiv",
    class: "div",
  });
  ...
  let editListBtnDiv = toolBox.func.addDiv($(editFrame), {
    id: "editListBtnDiv",
  });
  let editListBtnConfirm = toolBox.func.addButton($(editListBtnDiv), {
    class: "editBtn",
    id: "editListBtnConfirm",
  });
  $(editListBtnConfirm).text(" 確 認");
  let editListBtnCancel = toolBox.func.addButton($(editListBtnDiv), {
    class: "editBtn",
    id: " editListBtnCancel",
  });
  ...
  $("#editListBtnConfirm").on("click", function () {
    ...
    let valueWeight = $("#editWeightDiv")
      .find('input[name="editWeight"]')
      .val();
...
memberManager.func.showCalendarInfo = function (info) {
  ...
  let showInfoTitle = toolBox.func.addDiv($(showInfoFrame), {
    id: "showInfoTitle",
  });
  showInfoTitle.text("詳細資訊");
  let showInfoDivs = toolBox.func.addDiv($(showInfoFrame), {
    id: "showInfoDivs",
  });

  let showInfoDate = toolBox.func.addDiv($(showInfoDivs), {
    class: "showInfoDate",
  });
  $(showInfoDate).text(key);
  let showInfoTypeDiv = toolBox.func.addDiv($(showInfoDivs), {
    class: "showInfoDiv",
  });
  let showInfoTypeDivLab = toolBox.func.addDiv($(showInfoTypeDiv), {
    class: "showInfoTypeDivLab",
  });
  showInfoTypeDivLab.text("訓練部位 :");
  let showInfoTypeDivCon = toolBox.func.addDiv($(showInfoTypeDiv), {
    class: "showInfoTypeDivCon",
  });
  ...
  for (let i = 0; i < dataBaseArray.length; i++) {
    if (dataBaseArray[i].type == title) {
      let showInfoWayDiv = toolBox.func.addDiv($(showInfoDivs), {
        class: "showInfoDiv",
      });
      let showInfoWayDivLab = toolBox.func.addDiv($(showInfoWayDiv), {
        class: "showInfoTypeDivLab",
      });
      showInfoWayDivLab.text("訓練方式 :");
      let showInfoWayDivCon = toolBox.func.addDiv($(showInfoWayDiv), {
        class: "showInfoTypeDivCon",
      });
      showInfoWayDivCon.text(dataBaseArray[i].way);

      let showInfoWeightDiv = toolBox.func.addDiv($(showInfoDivs), {
        class: "showInfoDiv",
      });
      let showInfoWeightDivLab = toolBox.func.addDiv($(showInfoWeightDiv), {
        class: "showInfoTypeDivLab",
      });
      showInfoWeightDivLab.text("重量(公斤) :");
      let showInfoWeightDivCon = toolBox.func.addDiv($(showInfoWeightDiv), {
        class: "showInfoTypeDivCon",
      });
      showInfoWeightDivCon.text(dataBaseArray[i].weight);

      let showInfoRepsDiv = toolBox.func.addDiv($(showInfoDivs), {
        class: "showInfoDiv",
      });
      let showInfoRepsDivLab = toolBox.func.addDiv($(showInfoRepsDiv), {
        class: "showInfoTypeDivLab",
      });
      showInfoRepsDivLab.text("訓練次數 :");
      let showInfoRepsDivCon = toolBox.func.addDiv($(showInfoRepsDiv), {
        class: "showInfoTypeDivCon",
      });
      showInfoRepsDivCon.text(dataBaseArray[i].reps);

      let showInfoSetsDiv = toolBox.func.addDiv($(showInfoDivs), {
        class: "showInfoDiv",
      });
      let showInfoSetsDivLab = toolBox.func.addDiv($(showInfoSetsDiv), {
        class: "showInfoTypeDivLab",
      });
      showInfoSetsDivLab.text("訓練組數 :");
      let showInfoSetsDivCon = toolBox.func.addDiv($(showInfoSetsDiv), {
        class: "showInfoTypeDivCon",
      });
      showInfoSetsDivCon.text(dataBaseArray[i].sets);

      let showInfoRestDiv = toolBox.func.addDiv($(showInfoDivs), {
        class: "showInfoDiv",
      });
      let showInfoRestDivLab = toolBox.func.addDiv($(showInfoRestDiv), {
        class: "showInfoTypeDivLab",
      });
      showInfoRestDivLab.text("休息時間(秒數) :");
      let showInfoRestDivCon = toolBox.func.addDiv($(showInfoRestDiv), {
        class: "showInfoTypeDivCon",
      });
      showInfoRestDivCon.text(dataBaseArray[i].rest);
    }
  }
  let showInfoCloseBtn = toolBox.func.addButton($(showInfoFrame), {
    id: "showInfoCloseBtn",
  });
